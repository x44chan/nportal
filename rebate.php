...
<div id = "needaproval">
	...
	<div id = "reportg">
	<h4 style="margin-left: 20px;" style="visibility: hidden !important;" id = "xxhead"><i><u> Rebates </u></i></h4>
	...
	<?php		
		...
		//}else{
			$xxid = "";
		//}
		$sql = "SELECT * FROM rebate where  $xxid datefile BETWEEN '" . date("Y-m-d 00:00:00", strtotime($datefr)) . "' and '" . date("Y-m-d 23:59:59", strtotime($dateto)) . "'";
		...
		while($row = $result->fetch_assoc()){	
			...
			echo 
				'<tr>
					...
					<td>'. $row["note"] .'</td>';
				if(!isset($_GET['print'])){
				//	echo '<td><a href = "?datefr='. $datefr . '&dateto='. $dateto.'&reb_name='. $row['reb_name'] . '&id='.$row['rebate_id'].'&print" class = "btn btn-primary"> Print </a></td>';
				}
			echo '</tr>';
		}
			if(!isset($_GET['id'])){
		...
		<?php if(isset($_GET['reb_name']) && $_GET['reb_name'] != ""){ ?>
			<tr align="center" style="border-top: none !important;">
				<td><?php echo '<br><br><br>___________________________<br>'.$_GET['reb_name']; ?></td>
			</tr>
		<?php }
